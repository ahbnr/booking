...
import {CreateBooking, InteractionState, ViewingBookings} from "../InteractionState";
...
class TimeslotView extends React.Component<Properties, State> {
    ...
    onChangeCapacity(capacityChangeEvent: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) {
    ...
    createBooking() {
        if (this.state.timeslot != null) {
            this.props.changeInteractionState(
                new CreateBooking(this.state.timeslot)
            )
        }
    }
    ...
    viewBookings() {
        if (this.state.timeslot != null) {
            this.props.changeInteractionState(new ViewingBookings(this.state.timeslot))
        }
    }

    render() {
        if (this.state.timeslot == null || this.state.startTime == null || this.state.endTime == null) {
            ...
            return <>
                <MuiPickersUtilsProvider utils={LuxonUtils}>
                    <Fragment>
                        ...
                        <Button
                            disabled={this.state.timeslot.bookings.length >= this.state.timeslot.data.capacity}
                            onClick={this.createBooking}
                        >
                            {
                                this.state.timeslot.bookings.length < this.state.timeslot.data.capacity ?
                                    'Buchen'
                                    : 'Ausgebucht'
                            }
                        </Button>
